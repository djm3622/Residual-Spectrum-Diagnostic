...
def _train_neural_operator_surrogate_2d(
    ...
    def _capture_training_state(epoch_idx: int, val_loss_value: float) -> Dict[str, Any]:
        ...
        return {
            ...
            "optimizer_state": clone_state_value(optimizer.state_dict()),
            "scheduler_state": clone_state_value(scheduler.state_dict()) if scheduler is not None else None,
            "grad_scaler_state": clone_state_value(self.grad_scaler.state_dict()) if self.grad_scaler is not None else None,
            "rng_state": {
